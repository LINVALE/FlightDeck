...
test('Pause All belongs to the Wall, never an individual player Face', () => {
  assert.doesNotMatch(FACE, /pauseAllDoor|header-pause-all|function pauseAll\(|paintPauseAll/);
  assert.doesNotMatch(CSS, /\.pauseall/,
    'a house-wide action cannot float over an individual player composition');
});
...
test('ring artist view stays still and its exact sleeve always returns to album mode', () => {
  ...
  assert.match(rail, /\.shelf-transport \{\s*top: 45vh/,
    'the right chrome begins level with the top of the Dial ring');
  assert.match(rail, /\.shelf-volume \{\s*top: 57vh/);
  assert.match(rail, /\.shelf-browse \{\s*top: 69vh/,
    'volume and Browse keep their rhythm below the dial-aligned control row');
  assert.match(rail,
...
test('every Face names its room quietly at the bottom, and steps aside while the chrome shows it', () => {
  ...
  assert.match(FACE_SRC, /restRoom\.textContent = zone\.name;/, 'the whole zone name: grouped, the group as Roon names it');
